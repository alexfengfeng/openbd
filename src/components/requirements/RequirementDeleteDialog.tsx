...
export function RequirementDeleteDialog({
  ...
  const handleDelete = async () => {
    if (!requirement || !confirmName.trim()) {
      setError('请输入需求标题');
      return;
    }

    if (confirmName !== requirement.title) {
      setError('输入的标题与需求标题不匹配');
      return;
    }
    ...
    try {
      ...
      toast.success('需求已删除', {
        ...
        action: {
          ...
          onClick: async () => {
            try {
              const restoreResponse = await fetch(`/api/requirements/${requirement.id}/restore`, {
                method: 'POST',
              });
      ...
      <DialogContent className="sm:max-w-[425px]">
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="space-y-2">
            <Input
              ...
              placeholder={`输入 "${requirement?.title}" 确认删除`}
              onKeyDown={(e) => {
        ...
        <DialogFooter>
          ...
          <Button
            ...
            disabled={submitting || confirmName !== requirement?.title}
          >
            {submitting ? (
              <>
